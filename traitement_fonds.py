...
def load_file(path: str) -> pd.DataFrame:
    ...
    df["Matu_num"] = pd.to_numeric(
        df["Matu."].astype(str).str.replace("%","",regex=False).str.strip(), errors="coerce"
    ).fillna(0)
    df["Pipeline_pondere"] = df["Revenu_M"] * df["Matu_num"] / 100
    df["_date_derniere"]  = pd.to_datetime(df["Date de dernière activité"],  errors="coerce")
    df["_date_prochaine"] = pd.to_datetime(df["Date de prochaine activité"], errors="coerce")
...
if uploaded_main:
    ...
    if not os.path.exists("data.xlsx"):
        st.error("Aucun fichier trouvé. Uploade un fichier xlsx via la sidebar.")
        st.stop()
...
<div class="main-header">
    ...
    <p>Mis à jour le {today.strftime("%d %B %Y")} &nbsp;·&nbsp; {len(df)} leads filtrés sur {len(df_raw)}</p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    if uploaded_history:
        ...
        # Fichier actuel = aujourd'hui
        snapshots.append({"df": df_raw, "label": "Aujourd'hui", "ts": pd.Timestamp.now()})
...
with tab2:
    ...
    # ── Tableau natif avec column_config (largeurs) + colonne couleur indicateur
    ...
    def get_flag(row):
        ...
        if pd.notna(date_d) and date_d >= cutoff_derniere:
            return "🟢 Act. récente"
        if pd.notna(date_p) and now_ts <= date_p <= cutoff_prochaine:
            return "🟩 Relance proche"
        return ""

    df_disp.insert(0, "Alerte", df_disp.apply(get_flag, axis=1))
    ...
    # Injection CSS pour forcer wrap dans toutes les cellules
    st.markdown("""
    <style>
    ...
    st.dataframe(
        ...
        height=650,
        column_config={
            "Alerte":              st.column_config.TextColumn("",                    width=120),
            "Fonds":               st.column_config.TextColumn("Fonds",               width=140),
            "Typologie":           st.column_config.TextColumn("Typologie",           width=140),
            "Entité":              st.column_config.TextColumn("Entité",              width=160),
            "Contact":             st.column_config.TextColumn("Contact",             width=130),
            "Contact Levée":       st.column_config.TextColumn("Contact Levée",       width=130),
            "Dernier commentaire": st.column_config.TextColumn("Dernier commentaire", width=500),
            "Intérêt":             st.column_config.TextColumn("Intérêt",             width=80),
            "Dernière activité":   st.column_config.TextColumn("Dernière activité",   width=120),
            "Prochaine activité":  st.column_config.TextColumn("Prochaine activité",  width=120),
            "Ticket":              st.column_config.TextColumn("Ticket",              width=80),
            "Matu.":               st.column_config.TextColumn("Matu.",               width=65),
            "Tri":                 st.column_config.TextColumn("Tri",                 width=160),
        },
        hide_index=True,
    )
    # ── Export XLSX fidèle au format d'origine ───────────────────────────────────
    output = io.BytesIO()
    wb_src_path = "/tmp/data_main.xlsx" if uploaded_main else "data.xlsx"

    # On repart du fichier source pour garder le style d'origine
    try:
        from copy import deepcopy
        wb = load_workbook(wb_src_path)
        ws = wb.active

        # Lire les 2 premières lignes de titre (header=2 → rows 1 & 2 sont des titres)
        # On va créer un nouveau wb propre avec les colonnes voulues + couleurs

        # Colonnes du df_disp qu'on veut exporter
        ...
        font_dark  = Font(color="1A1A2E", size=9)
        ...
        ws_out.title = "Pipeline"

        # En-tête
        for ci, col in enumerate(export_cols, 1):
            cell = ws_out.cell(row=1, column=ci, value=col)
            cell.fill   = fill_header
            cell.font   = font_white
            cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
            cell.border = border

        # Largeurs colonnes (plus généreuses)
        col_widths = {
            "📌":4,"Fonds":26,"Typologie":24,"Entité":28,"Contact":22,"Contact Levée":22,
            "Dernier commentaire":70,"Intérêt":12,
            "Dernière activité":18,"Prochaine activité":18,
            "Ticket":13,"Matu.":10,"Tri":24
        }
        for ci, col in enumerate(export_cols, 1):
            ws_out.column_dimensions[get_column_letter(ci)].width = col_widths.get(col, 18)

        # Gèle la ligne d'en-tête
        ...
        for ri, (_, row) in enumerate(df_disp.iterrows(), 2):
            ...
            for ci, col in enumerate(export_cols, 1):
                ...
                cell.border    = border
                cell.alignment = align_wrap if col == "Dernier commentaire" else align_top
                ...
                elif flag_p and col in ["Prochaine activité","Tri","Dernier commentaire"]:
                    cell.fill = fill_light
                    cell.font = font_dark
                elif ri % 2 == 0:
                    cell.fill = PatternFill("solid", fgColor="F8F9FC")

            ws_out.row_dimensions[ri].height = 80
